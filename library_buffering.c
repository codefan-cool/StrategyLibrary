...
{  
  ...
  float sma = calcSMA(bollingerInputPriceArray);
  bollingerSMA >> sma;
  bollingerSTDDEV = calcStdDev(bollingerSMA, bollingerInputPriceArray);
  float upper = calcBollingerUpperBand(bollingerSMA, bollingerSTDDEV, deviation);
  bollingerUpperBand >> upper;
  float lower = calcBollingerLowerBand(bollingerSMA, bollingerSTDDEV, deviation);
  bollingerLowerBand >> lower;

  print("Initial SMA :" + toString(sma));
  print("Initial bollingerSTDDEV :" + toString(bollingerSTDDEV));
  print("Initial bollingerUpperBand :" + toString(upper));
  print("Initial bollingerLowerBand :" + toString(lower));
...
void bollingerBandsBackTestTick()
{
  ...
  if (((bollingerBackTestTickCounter+1) % step) == 0)   // Update bollinger bands
  {
    ...
    float sma = calcSMA(bollingerInputPriceArray);
    bollingerSMA >> sma;
    bollingerSTDDEV = calcStdDev(sma, bollingerInputPriceArray);
    float upper = calcBollingerUpperBand(sma, bollingerSTDDEV, bollingerSettingDeviation);
    bollingerUpperBand >> upper;
    float lower = calcBollingerLowerBand(sma, bollingerSTDDEV, bollingerSettingDeviation);
    bollingerlowerBand >> lower;

    // print("New SMA :" + toString(bollingerSMA));

    // drawing new bollinger band's points
    if ((bollingerBackTestTickCounter+1) % 150 == 0)
    {
      setLineName("middle");
      setLineColor("yellow");
      for (integer i=0; i<sizeof(bollingerSMA); i++)
      {
        drawLine(lookbackTransactions[0].tradeTime, bollingerSMA[i]);
      ...
      for (integer i=0; i<sizeof(bollingerSMA); i++)
      {      
        drawLine(lookbackTransactions[0].tradeTime, bollingerUpperBand[i]);
      }
      ...
      for (integer i=0; i<sizeof(bollingerSMA); i++)
      {      
        drawLine(lookbackTransactions[0].tradeTime, bollingerLowerBand[i]);      
      }
    }
  }
  ...
  if (sizeof(lookbackTransactions) == 1)
  {
    ...
    // float lastBalanceInLTC = balanceLTC + (balanceBTC / lastPrice);
    // float lastBalanceInBTC = balanceBTC + (balanceLTC * lastPrice);
    // print("Last price is " + toString(lastPrice));
...
bollingerBandsBackTest("Centrabit", "LTC/BTC", 20, 2.0, "1h", 1.0, "2020-11-08 00:00:00", "2020-11-08 20:00:00");
stopLoss(0.008);